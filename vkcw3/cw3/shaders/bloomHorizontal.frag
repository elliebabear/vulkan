...
layout( set = 0, binding = 0 ) uniform sampler2D brightTex;
...
void main()
{
    ...
    for(int i = 0; i < 11; ++i)
    {
        offset[i] = ( texOffset.x*(2*i)*weights[2*i]+texOffset.x*(2*i+1)*weights[2*i+1] )/ LinearWeights[i];
    }

    vec3 result = texture(brightTex, v2fUV).rgb * LinearWeights[0];

    for(int i = 1; i < 11; ++i)
    {
        result += texture(brightTex, v2fUV + vec2(offset[i], 0.0)).rgb * LinearWeights[i];
        result += texture(brightTex, v2fUV - vec2(offset[i], 0.0)).rgb * LinearWeights[i];
    }
