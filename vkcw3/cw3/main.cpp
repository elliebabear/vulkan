...
#		define SHADERDIR_ "assets/cw3/shaders/"
		...
		constexpr char const* kFinalVertShaderPath = SHADERDIR_ "fullscreen.vert.spv";
		constexpr char const* kFinalFragShaderPath = SHADERDIR_ "fullscreen.frag.spv";
		constexpr char const* kbhFragShaderPath = SHADERDIR_ "bloomHorizontal.frag.spv";
		constexpr char const* kbvFragShaderPath = SHADERDIR_ "bloomVertical.frag.spv";

#		undef SHADERDIR_

#		define ASSETDIR_ "assets/cw3/"
		constexpr char const* kBakedModelPath = ASSETDIR_"suntemple.comp5822mesh";
#		undef ASSETDIR_
	...
	void update_scene_uniforms(glsl::SceneUniform&, std::uint32_t aFramebufferWidth, std::uint32_t aFramebufferHeight, UserState const&);

	lut::RenderPass create_render_passA(lut::VulkanWindow const& aWindow);
	lut::RenderPass create_render_passB(lut::VulkanWindow const& aWindow);
	lut::RenderPass create_bloom_render_pass(lut::VulkanWindow const& aWindow);
	...
	lut::PipelineLayout create_pipeline_singleLayout(lut::VulkanContext const& aContext, VkDescriptorSetLayout firstLayout);
	lut::PipelineLayout create_pipeline_doubleLayout(lut::VulkanContext const& aContext, VkDescriptorSetLayout firstLayout, VkDescriptorSetLayout secondLayout);

	lut::Pipeline create_pipelineA(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout);
	lut::Pipeline create_pipelineB(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout);
	lut::Pipeline create_bloom_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout, const char* VertShaderPath, const char* FragShaderPath);
	...
	lut::DescriptorSetLayout create_single_descriptor_layout(lut::VulkanWindow const&);
	...
	void create_intermediate_framebuffers(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, lut::Framebuffer& aFramebuffers, VkImageView aBaseView, VkImageView aBrightView, VkImageView aDepthView);
	void create_bloom_framebuffers(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, lut::Framebuffer& aFramebuffers, VkImageView aBloomView);

	void record_intermediate_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPassA, VkFramebuffer aIntFramebuffer, VkPipeline aGraphicsPipe,
		VkExtent2D const& aImageExtent, VkBuffer aSceneUBO, glsl::SceneUniform const& aSceneUniform, VkPipelineLayout aGraphicsLayout,
		VkDescriptorSet aSceneDescriptors, std::vector<objMesh>& aModel, VkRenderPass abhPass, VkFramebuffer abhFramebuffer, VkPipeline abhPipe,
		VkPipelineLayout aBloomPipeLayout, VkDescriptorSet abrightDesc, VkRenderPass abvPass, VkFramebuffer abvFramebuffer, VkPipeline abvPipe, VkDescriptorSet abhDesc);

	void record_final_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPassB, VkFramebuffer aFramebuffer, VkPipeline afinalPipe,
		VkExtent2D const& aImageExtent, VkPipelineLayout afinalPipeLayout, VkDescriptorSet abaseDesc, VkDescriptorSet abrightDesc);

	void submit_intermediate_commands(lut::VulkanWindow const& aWindow, VkCommandBuffer aCmdBuff, VkFence aFence);

	void submit_final_commands(lut::VulkanWindow const& aWindow, VkCommandBuffer aCmdBuff, VkFence aFence, VkSemaphore aWaitSemaphore, VkSemaphore aSignalSemaphore);
...
int main() try
{
	...
	// Intialize resources
	lut::RenderPass renderPassA = create_render_passA(window);
	lut::RenderPass renderPassB = create_render_passB(window);//finalpass
	lut::RenderPass bhPass = create_bloom_render_pass(window);
	lut::RenderPass bvPass = create_bloom_render_pass(window);
	...
	//CW3 : NEED MORE
	lut::DescriptorSetLayout singleLayout = create_single_descriptor_layout(window);



	lut::PipelineLayout pipeLayout = create_pipeline_layout(window, sceneLayout.handle, objectLayout.handle);
	lut::Pipeline pipe = create_pipelineA(window, renderPassA.handle, pipeLayout.handle);
	//CW3 : NEED MORE
	lut::PipelineLayout finalPipeLayout = create_pipeline_doubleLayout(window, singleLayout.handle, singleLayout.handle);
	lut::Pipeline finalPipe = create_pipelineB(window, renderPassB.handle, finalPipeLayout.handle);

	lut::PipelineLayout bloomPipeLayout = create_pipeline_singleLayout(window, singleLayout.handle);
	lut::Pipeline bhPipe = create_bloom_pipeline(window, bhPass.handle, bloomPipeLayout.handle, cfg::kFinalVertShaderPath, cfg::kbhFragShaderPath);
	lut::Pipeline bvPipe = create_bloom_pipeline(window, bvPass.handle, bloomPipeLayout.handle, cfg::kFinalVertShaderPath, cfg::kbvFragShaderPath);



	//intermediate textures
	lut::Image baseImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height,VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
	lut::ImageView baseView = lut::create_image_view_texture2d(window, baseImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);

	lut::Image brightImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height,VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
	lut::ImageView brightView = lut::create_image_view_texture2d(window, brightImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);

	lut::Image bhImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
	lut::ImageView bhView = lut::create_image_view_texture2d(window, bhImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);

	lut::Image bvImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
	lut::ImageView bvView = lut::create_image_view_texture2d(window, bvImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);
	...
	lut::Framebuffer toneFramebuffer, bhFramebuffer, bvFramebuffer;
	create_intermediate_framebuffers(window, renderPassA.handle, toneFramebuffer, baseView.handle, brightView.handle, depthBufferView.handle);
	create_bloom_framebuffers(window, bhPass.handle, bhFramebuffer, bhView.handle);
	create_bloom_framebuffers(window, bvPass.handle, bvFramebuffer, bvView.handle);

	lut::CommandPool cpool = lut::create_command_pool(window, VK_COMMAND_POOL_CREATE_TRANSIENT_BIT | VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);

	VkCommandBuffer tcbuBuffer = lut::alloc_command_buffer(window, cpool.handle);
	lut::Fence tcbFence = lut::create_fence(window, 0);
	...
	std::vector<objMesh> cw3Model = create_mesh(window, allocator, BModel, loadCmdPool.handle, dpool.handle, defaultSampler.handle, objectLayout.handle);
	...
	VkDescriptorSet baseTexDesc = lut::alloc_desc_set(window, dpool.handle, singleLayout.handle);
	{
		VkWriteDescriptorSet desc[1]{};
		VkDescriptorImageInfo imageInfo[1]{};
		imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[0].imageView = baseView.handle;
		imageInfo[0].sampler = defaultSampler.handle;
		desc[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[0].dstSet = baseTexDesc;
		desc[0].dstBinding = 0;
	...
	VkDescriptorSet brightTexDesc = lut::alloc_desc_set(window, dpool.handle, singleLayout.handle);
	{
		VkWriteDescriptorSet desc[1]{};
		VkDescriptorImageInfo imageInfo[1]{};
		imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[0].imageView = brightView.handle;
		imageInfo[0].sampler = defaultSampler.handle;
		desc[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[0].dstSet = brightTexDesc;
		desc[0].dstBinding = 0;
		desc[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		desc[0].descriptorCount = 1;
		desc[0].pImageInfo = &imageInfo[0];
	...
	VkDescriptorSet bhTexDesc = lut::alloc_desc_set(window, dpool.handle, singleLayout.handle);
	{
		VkWriteDescriptorSet desc[1]{};
		VkDescriptorImageInfo imageInfo[1]{};
		imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[0].imageView = bhView.handle;
		imageInfo[0].sampler = defaultSampler.handle;
		desc[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[0].dstSet = bhTexDesc;
		desc[0].dstBinding = 0;
		desc[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		desc[0].descriptorCount = 1;
		desc[0].pImageInfo = &imageInfo[0];
	...
	VkDescriptorSet bvTexDesc = lut::alloc_desc_set(window, dpool.handle, singleLayout.handle);
	{
		VkWriteDescriptorSet desc[1]{};
		VkDescriptorImageInfo imageInfo[1]{};
		imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[0].imageView = bvView.handle;
		imageInfo[0].sampler = defaultSampler.handle;
		desc[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		desc[0].dstSet = bvTexDesc;
		desc[0].dstBinding = 0;
		desc[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		desc[0].descriptorCount = 1;
		desc[0].pImageInfo = &imageInfo[0];
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		if (recreateSwapchain)
		{
			...
			{
				renderPassA = create_render_passA(window);
				renderPassB = create_render_passB(window);
				bhPass = create_bloom_render_pass(window);
				bvPass = create_bloom_render_pass(window);
			}
			if (changes.changedSize)
			{
				std::tie(depthBuffer, depthBufferView) = create_depth_buffer(window, allocator);

				baseImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
				baseView = lut::create_image_view_texture2d(window, baseImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);

				brightImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
				brightView = lut::create_image_view_texture2d(window, brightImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);

				bhImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
				bhView = lut::create_image_view_texture2d(window, bhImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);

				bvImage = lut::create_intermediate_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
				bvView = lut::create_image_view_texture2d(window, bvImage.image, VK_FORMAT_R16G16B16A16_SFLOAT);
			}
			create_intermediate_framebuffers(window, renderPassA.handle, toneFramebuffer, baseView.handle, brightView.handle, depthBufferView.handle);
			create_bloom_framebuffers(window, bhPass.handle, bhFramebuffer, bhView.handle);
			create_bloom_framebuffers(window, bvPass.handle, bvFramebuffer, bvView.handle);
			...
			if (changes.changedSize)
			{
				pipe = create_pipelineA(window, renderPassA.handle, pipeLayout.handle);
				finalPipe = create_pipelineB(window, renderPassB.handle, finalPipeLayout.handle);
				bhPipe = create_bloom_pipeline(window, bhPass.handle, bloomPipeLayout.handle, cfg::kFinalVertShaderPath, cfg::kbhFragShaderPath);
				bvPipe = create_bloom_pipeline(window, bvPass.handle, bloomPipeLayout.handle, cfg::kFinalVertShaderPath, cfg::kbvFragShaderPath);
			}
		...
		record_intermediate_commands(tcbuBuffer, renderPassA.handle, toneFramebuffer.handle, pipe.handle, window.swapchainExtent, sceneUBO.buffer, sceneUniforms, pipeLayout.handle, sceneDescriptors, cw3Model,
			bhPass.handle, bhFramebuffer.handle, bhPipe.handle, bloomPipeLayout.handle, brightTexDesc, bvPass.handle, bvFramebuffer.handle, bvPipe.handle, bhTexDesc);

		submit_intermediate_commands(window, tcbuBuffer, tcbFence.handle);

		if (auto const res = vkWaitForFences(window.device, 1, &tcbFence.handle, VK_TRUE, std::numeric_limits<std::uint64_t>::max()); VK_SUCCESS != res)
		{
			throw lut::Error("Waiting for upload to complete\n" "vkWaitForFences() returned %s", lut::to_string(res).c_str());
		}
		if (auto const res = vkResetFences(window.device, 1, &tcbFence.handle); VK_SUCCESS != res)
		{
		...
		record_final_commands(cbuffers[imageIndex], renderPassB.handle, framebuffers[imageIndex].handle, finalPipe.handle, window.swapchainExtent, finalPipeLayout.handle, baseTexDesc, bvTexDesc);

		submit_final_commands(window, cbuffers[imageIndex], cbfences[imageIndex].handle, imageAvailable.handle, renderFinished.handle);
...
namespace
{
	void update_scene_uniforms(glsl::SceneUniform& aSceneUniforms, std::uint32_t aFramebufferWidth, std::uint32_t aFramebufferHeight, UserState const& aState)
	{
		...
		aSceneUniforms.lightColor = aState.LC;
...
namespace
{
	lut::RenderPass create_render_passA(lut::VulkanWindow const& aWindow)
	{
		...
		attachments[0].format = VK_FORMAT_R16G16B16A16_SFLOAT;  //HDR
		...
		subpassAttachments[1].attachment = 1; // the zero refers to attachments[0] declared earlier.
		subpassAttachments[1].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

		VkAttachmentReference depthAttachment{};
		depthAttachment.attachment = 2; // this refers to attachments[1]
	...
	lut::Pipeline create_pipelineA(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		...
		// Define blend state:
		// The size depends on the number of color attachments in render pass.
		VkPipelineColorBlendAttachmentState blendStates[2]{};//cw3
	...
	{
		//TODO: implement me!
		lut::ShaderModule vert = lut::load_shader_module(aWindow, cfg::kFinalVertShaderPath);
		lut::ShaderModule frag = lut::load_shader_module(aWindow, cfg::kFinalFragShaderPath);
		...
		rasterInfo.polygonMode = VK_POLYGON_MODE_FILL;
		rasterInfo.cullMode = VK_CULL_MODE_BACK_BIT;
		rasterInfo.frontFace = VK_FRONT_FACE_CLOCKWISE;// cw3----VK_FRONT_FACE_COUNTER_CLOCKWISE
		rasterInfo.depthBiasEnable = VK_FALSE;
		rasterInfo.lineWidth = 1.f;
		...
		VkPipelineColorBlendAttachmentState blendStates[1]{};//cw3
		blendStates[0].colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
		blendStates[0].blendEnable = VK_FALSE;
	...
	lut::DescriptorSetLayout create_single_descriptor_layout(lut::VulkanWindow const& aWindow)
	{
		VkDescriptorSetLayoutBinding bindings[1]{};
		bindings[0].binding = 0;
		bindings[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	...
	void record_intermediate_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPassA, VkFramebuffer aIntFramebuffer, VkPipeline aGraphicsPipe,
		VkExtent2D const& aImageExtent, VkBuffer aSceneUBO, glsl::SceneUniform const& aSceneUniform, VkPipelineLayout aGraphicsLayout,
		VkDescriptorSet aSceneDescriptors, std::vector<objMesh>& aModel, VkRenderPass abhPass, VkFramebuffer abhFramebuffer, VkPipeline abhPipe,
		VkPipelineLayout aBloomPipeLayout, VkDescriptorSet abrightDesc, VkRenderPass abvPass, VkFramebuffer abvFramebuffer, VkPipeline abvPipe, 
		VkDescriptorSet abhDesc)
	{
		...
		//Begin render pass
		{
			...
			passInfo.framebuffer = aIntFramebuffer;
		...
		//horizontal 
		{
			...
			VkRenderPassBeginInfo passInfo{};
			passInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
			passInfo.renderPass = abhPass;
			passInfo.framebuffer = abhFramebuffer;
			passInfo.renderArea.offset = VkOffset2D{ 0, 0 };
			passInfo.renderArea.extent = aImageExtent;
			...
			//Begin drawing with our graphics pipeline
			vkCmdBindPipeline(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, abhPipe);
			vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aBloomPipeLayout, 0, 1, &abrightDesc, 0, nullptr);
		...
		//vertical
		{
			...
			VkRenderPassBeginInfo passInfo{};
			passInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
			passInfo.renderPass = abvPass;
			passInfo.framebuffer = abvFramebuffer;
			passInfo.renderArea.offset = VkOffset2D{ 0, 0 };
			passInfo.renderArea.extent = aImageExtent;
			passInfo.clearValueCount = 1;
			passInfo.pClearValues = clearValues;
			...
			//Begin drawing with our graphics pipeline
			vkCmdBindPipeline(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, abvPipe);
			vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aBloomPipeLayout, 0, 1, &abhDesc, 0, nullptr);
			vkCmdDraw(aCmdBuff, 3, 1, 0, 0);
			vkCmdEndRenderPass(aCmdBuff);
		}
		...
		VkExtent2D const& aImageExtent, VkPipelineLayout afinalPipeLayout, VkDescriptorSet abaseDesc, VkDescriptorSet abrightDesc)
	{
		...
		//Begin drawing with our graphics pipeline
		vkCmdBindPipeline(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, afinalPipe);
		vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, afinalPipeLayout, 0, 1, &abaseDesc, 0, nullptr);
		vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, afinalPipeLayout, 1, 1, &abrightDesc, 0, nullptr);
		vkCmdDraw(aCmdBuff, 3, 1, 0, 0);
		vkCmdEndRenderPass(aCmdBuff);
		...
		VkCommandPool& aLoadCmdPool, VkDescriptorPool& aDesPool, VkSampler& aSampler, VkDescriptorSetLayout& aLayout)
	{
		...
		for (auto mesh : aModel.meshes)
		{
			...
			textureInfo[4].imageView = imageViews[aModel.materials[mesh.materialId].emissiveTextureId].handle;
			textureInfo[4].sampler = aSampler;
