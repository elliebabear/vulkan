...
namespace labutils
{
	...
	Image load_normal_image_texture2d(char const* aPath, VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator)
	{
		//throw Error( "Not yet implemented" ); //TODO- (Section 4) implement me!
		stbi_set_flip_vertically_on_load(1);
		//load base image
		...
		vmaUnmapMemory(aAllocator.allocator, staging.allocation);
		stbi_image_free(data);
		Image ret = create_image_texture2d(aAllocator, baseWidth, baseHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
		VkCommandBuffer cbuff = alloc_command_buffer(aContext, aCmdPool);
		VkCommandBufferBeginInfo beginInfo{};
		beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
		beginInfo.flags = 0;
	...
	Image load_dummy_image_texture2d(VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator)
	{
		uint32_t baseHeight = 1;
		uint32_t baseWidth = 1;
		uint32_t baseChannel = 4;

		std::uint8_t data[4] = { 128, 128, 255, 255 };

		auto const sizeInBytes = baseHeight * baseWidth * baseChannel;
	...
	Image create_intermediate_image_texture2d(Allocator const& aAllocator, std::uint32_t aWidth, std::uint32_t aHeight, VkFormat aFormat, VkImageUsageFlags aUsage)
	{
		VkImageCreateInfo imageInfo{};
		imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
		imageInfo.imageType = VK_IMAGE_TYPE_2D;
