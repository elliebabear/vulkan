...
#		undef ASSETDIR_
		...
		constexpr VkFormat kShadowMapFormat = VK_FORMAT_D16_UNORM;
		...
		constexpr std::uint32_t kShadowMapRes = 1024;
	}
	...
	struct  UserState
	{
		...
		glm::vec3 light_pos = glm::vec3(0.f, 5.f, 2.f);
	};
	...
	namespace glsl
	{
		...
		struct  lightPerspectiveSpaceUniform
		{
	...
	lut::RenderPass create_shadowMap_pass(lut::VulkanWindow const&);
	...
	lut::PipelineLayout create_shadowMap_pipeline_layout(lut::VulkanContext const&, VkDescriptorSetLayout);
	...
	std::tuple<lut::Image, lut::ImageView> create_shadowMap_image_and_view(lut::VulkanWindow const& aWindow, lut::Allocator const& aAllocator);

	void create_shadowMap_framebuffer(lut::VulkanWindow const&, VkRenderPass, lut::Framebuffer&, VkImageView aDepthView);
	...
	void update_scene_uniforms(
		glsl::SceneUniform&,
		glsl::lightPerspectiveSpaceUniform&,
	...
	void record_commands(
		...
		VkDescriptorSet aShadowMapLightDescriptor
	);

	void record_shadowmap_pass_commands(
		VkCommandBuffer aCmdBuff,
		VkRenderPass aRenderPass,
		VkFramebuffer aFramebuffer,
		VkPipeline aPipeline,
		VkBuffer aLightUBO,
		glsl::lightPerspectiveSpaceUniform const& aLightUniform,
		VkPipelineLayout,
		VkDescriptorSet aLightDescriptor,
		ModelPack& aModel
	);
...
int main() try
{
	...
	lut::RenderPass shadowMapPass = create_shadowMap_pass(window); 
	...
	lut::DescriptorSetLayout shadowMapLightLayout = create_scene_descriptor_layout(window);
	lut::DescriptorSetLayout shadowMapLayout = create_shadow_descriptor_layout(window);

	lut::PipelineLayout pipeLayout = create_pipeline_layout(window, sceneLayout.handle, objectLayout.handle, shadowMapLayout.handle, shadowMapLightLayout.handle);
	lut::Pipeline pipe = create_pipeline(window, renderPass.handle, pipeLayout.handle);

	lut::PipelineLayout shadowMapPipeLayout = create_shadowMap_pipeline_layout(window, shadowMapLightLayout.handle);
	lut::Pipeline shadowMapPipe = create_shadowMap_pipeline(window, shadowMapPass.handle, shadowMapPipeLayout.handle);
	

	auto [shadowMapImage, shadowMapView] = create_shadowMap_image_and_view(window, allocator);

	auto [depthBuffer, depthBufferView] = create_depth_buffer(window, allocator);

	lut::Framebuffer shadowMapFramebuffer;
	create_shadowMap_framebuffer(window, shadowMapPass.handle, shadowMapFramebuffer, shadowMapView.handle); 
	...
	VkCommandBuffer shadowMapCmdBuffer = lut::alloc_command_buffer(window, cpool.handle);
	lut::Fence shadowMapFence = lut::create_fence(window, 0);
	...
	lut::Buffer lightUBO = lut::create_buffer(allocator, sizeof(glsl::lightPerspectiveSpaceUniform),
	...
	VkDescriptorSet lightDescriptor = lut::alloc_desc_set(window, dPool.handle, shadowMapLightLayout.handle);
	...
	lut::Sampler shadowMapSampler = lut::create_shadowMap_sampler(window);
	VkDescriptorSet ShadowMapDescriptor = lut::alloc_desc_set(window, dPool.handle, shadowMapLayout.handle);

	{
		VkWriteDescriptorSet desc[1]{};
		VkDescriptorImageInfo imageInfo[1]{};
		imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[0].imageView = shadowMapView.handle;
		imageInfo[0].sampler = shadowMapSampler.handle; 
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		glsl::lightPerspectiveSpaceUniform lightUniforms{}; 
		update_scene_uniforms(sceneUniforms, lightUniforms, window.swapchainExtent.width, window.swapchainExtent.height, state);

		record_shadowmap_pass_commands(shadowMapCmdBuffer, shadowMapPass.handle, shadowMapFramebuffer.handle, shadowMapPipe.handle,
			lightUBO.buffer, lightUniforms, shadowMapPipeLayout.handle, lightDescriptor, ourModel);
		submit_commandsA(window, shadowMapCmdBuffer, shadowMapFence.handle);

		if (auto const res = vkWaitForFences(window.device, 1, &shadowMapFence.handle, VK_TRUE, std::numeric_limits<std::uint64_t>::max()); VK_SUCCESS != res)
		{
			throw lut::Error("Waiting for upload to complete\n" "vkWaitForFences() returned %s", lut::to_string(res).c_str());
		}
		if (auto const res = vkResetFences(window.device, 1, &shadowMapFence.handle); VK_SUCCESS != res)
		{
		...
		record_commands(cbuffers[imageIndex], renderPass.handle, framebuffers[imageIndex].handle, pipe.handle,
			window.swapchainExtent, sceneUBO.buffer, sceneUniforms, pipeLayout.handle, sceneDescriptors, ourModel, bakedModel, ShadowMapDescriptor, lightDescriptor);
...
namespace
{
	void update_scene_uniforms(glsl::SceneUniform& aSceneUniforms, glsl::lightPerspectiveSpaceUniform& aLightUniforms, std::uint32_t aFramebufferWidth, std::uint32_t aFramebufferHeight, UserState aState)
	{
		...
		glm::vec3 sceneCenter = glm::vec3(0, 0, -100);
		float lightFOV = lut::Radians(90.0_degf).value();
		glm::mat4 lightView = glm::lookAt(aState.light_pos, sceneCenter, glm::vec3(0.0f, 1.0f, 0.0f));
		glm::mat4 lightProj = glm::perspectiveRH_ZO(lightFOV, 1.0f, cfg::kCameraNear, cfg::kCameraFar);
...
namespace
{
	...
	lut::RenderPass create_shadowMap_pass(lut::VulkanWindow const& aWindow)
	{
		...
		VkAttachmentReference depthAttachment{};
		depthAttachment.attachment = 0; //this refers to attachments[0]
		depthAttachment.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
	...
	lut::PipelineLayout create_pipeline_layout(lut::VulkanContext const& aContext, VkDescriptorSetLayout aSceneLayout, VkDescriptorSetLayout aObjectLayout, VkDescriptorSetLayout aShadowMapLayout, VkDescriptorSetLayout aLightLayout)
	{
		//TODO- (Section 1 / Exercise 3) implement me!
	...
	lut::Pipeline create_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		...
		//rasterInfo.depthBiasConstantFactor = 0.2f;
	...
	lut::PipelineLayout create_shadowMap_pipeline_layout(lut::VulkanContext const& aContext, VkDescriptorSetLayout aLightLayout)
	{
		VkDescriptorSetLayout layouts[] =
		{  //Order must match the set = N in the shaders
			aLightLayout ,//set 0
	...
	lut::Pipeline create_shadowMap_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		...
		VkVertexInputAttributeDescription vertexAttribs[1]{};
		...
		viewport.x = 0.f;
		viewport.y = 0.f;
		viewport.width = cfg::kShadowMapRes;
		viewport.height = cfg::kShadowMapRes;
		viewport.minDepth = 0.f;
		viewport.maxDepth = 1.f;

		VkRect2D scissor{};
		scissor.offset = VkOffset2D{ 0,0 };
		scissor.extent.width = cfg::kShadowMapRes;
		scissor.extent.height = cfg::kShadowMapRes;
	...
	std::tuple<lut::Image, lut::ImageView> create_shadowMap_image_and_view(lut::VulkanWindow const& aWindow, lut::Allocator const& aAllocator)
	{
		...
		imgInfo.imageType = VK_IMAGE_TYPE_2D;
		imgInfo.format = cfg::kDepthFormat;
		imgInfo.extent.width = cfg::kShadowMapRes;
		imgInfo.extent.height = cfg::kShadowMapRes;
		imgInfo.extent.depth = 1;
		imgInfo.mipLevels = 1;
		imgInfo.arrayLayers = 1;
		imgInfo.samples = VK_SAMPLE_COUNT_1_BIT;
	...
	void create_shadowMap_framebuffer(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, lut::Framebuffer& aFramebuffer, VkImageView aDepthView)
	{
		VkImageView attachments[1] = {aDepthView };
		...
		fbInfo.width = cfg::kShadowMapRes;
		fbInfo.height = cfg::kShadowMapRes;
		fbInfo.layers = 1;
		...
		aFramebuffer = std::move(lut::Framebuffer(aWindow.device, fb));
	}
	...
	void record_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPass, VkFramebuffer aFramebuffer,
		...
		VkPipelineLayout aGraphicsLayout, VkDescriptorSet aSceneDescriptors, ModelPack& aModel, BakedModel& bakedModel, VkDescriptorSet aShadowMapDescriptor, VkDescriptorSet aShadowMapLightDescriptor)
	{
		...
		vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout, 2, 1, &aShadowMapDescriptor, 0, nullptr);
		vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout, 3, 1, &aShadowMapLightDescriptor, 0, nullptr); 
	...
	void record_shadowmap_pass_commands(VkCommandBuffer aCmdBuff, VkRenderPass aRenderPass, VkFramebuffer aFramebuffer, 
		VkPipeline aPipeline, VkBuffer aLightUBO, glsl::lightPerspectiveSpaceUniform const& aLightUniform, 
		VkPipelineLayout aPipelineLayout, VkDescriptorSet aLightDescriptor, ModelPack& aModel)
	{
		//Begin recording commands
		VkCommandBufferBeginInfo begInfo{};
		begInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
		...
		//Upload scene uniforms
		lut::buffer_barrier(aCmdBuff, aLightUBO, VK_ACCESS_UNIFORM_READ_BIT, VK_ACCESS_TRANSFER_WRITE_BIT,
			VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

		vkCmdUpdateBuffer(aCmdBuff, aLightUBO, 0, sizeof(glsl::lightPerspectiveSpaceUniform), &aLightUniform);

		lut::buffer_barrier(aCmdBuff, aLightUBO, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_UNIFORM_READ_BIT,
			VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT);
		...
		passInfo.framebuffer = aFramebuffer;
		passInfo.renderArea.offset = VkOffset2D{ 0, 0 };
		passInfo.renderArea.extent.width = cfg::kShadowMapRes;
		passInfo.renderArea.extent.height = cfg::kShadowMapRes;
		...
		vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aPipelineLayout, 0, 1, &aLightDescriptor, 0, nullptr);
		for (auto& mesh : aModel.meshes)
		{
			VkDeviceSize offsets[1]{};
			vkCmdBindVertexBuffers(aCmdBuff, 0, 1, &mesh.vertices.buffer, offsets);
