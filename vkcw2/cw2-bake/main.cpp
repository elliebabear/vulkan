...
namespace
{
	void process_model_( char const* aOutput, char const* aInputOBJ, glm::mat4x4 const& aStaticTransform )
	{
		...
		// Index meshes
		auto const indexed = index_meshes_( model );//return type - std::vector<IndexedMesh>
		...
		// return value - std::unordered_map<std::string,TextureInfo_>
...
namespace
{
	...
	void write_model_data_( FILE* aOut, InputModel const& aModel, std::vector<IndexedMesh> const& aIndexedMeshes, std::unordered_map<std::string,TextureInfo_> const& aTextures )
	{
		...
		checked_write_( aOut, sizeof(textureCount), &textureCount );//  - unit32_t : U = number of unique textures

		for( auto const& tex : orderedUnqiue )
		{
			assert( tex );
			write_string_( aOut, tex->newPath.c_str() );//    - string : path to texture 

			std::uint8_t channels = tex->channels;
			checked_write_( aOut, sizeof(channels), &channels );//    - uint8_t : number of channels in texture
		}
		...
		checked_write_( aOut, sizeof(materialCount), &materialCount );//  - uint32_t : M = number of materials
		...
		for( auto const& mat : aModel.materials )
		{
			auto const write_tex_ = [&] (std::string const& aTexturePath ) {
				...
				checked_write_( aOut, sizeof(std::uint32_t), &it->second.uniqueId );// write xxx texture index
			};//little local function: write_tex_(...filePath...)
		...
		checked_write_( aOut, sizeof(meshCount), &meshCount );//  - uint32_t : M = number of meshes
